...
class EvidencijaApp:
    ...
    def spremi_u_xml(self):
        ...
        for ucenik in Ucenik.ucenici:
            ucenik_element = ET.SubElement(root, "ucenik")
            ime_element = ET.SubElement(ucenik_element, "ime")
            ime_element.text = ucenik.ime
            prezime_element = ET.SubElement(ucenik_element, "prezime")
            prezime_element.text = ucenik.prezime
            razred_element = ET.SubElement(ucenik_element, "razred")
            razred_element.text = ucenik.razred
        xml_string = ET.tostring(root)
        xml_pretty = minidom.parseString(xml_string).toprettyxml(indent="  ")
        with open("ucenici.xml", "w") as xmlfile:
            xmlfile.write(xml_pretty)  
    def ucitaj_iz_csv(self):
        Ucenik.ucenici = []
        with open("ucenici.csv", "r") as csvfile:
            reader = csv.reader(csvfile)
            next(reader)
            for row in reader:
                ime, prezime, razred = row
                ucenik = Ucenik(ime, prezime, razred)
                Ucenik.ucenici.append(ucenik)
        self.osvjezi_listu_ucenika()    
    def ucitaj_iz_xml(self):
        Ucenik.ucenici = []
        tree = ET.parse("ucenici.xml")
        root = tree.getroot()
        for ucenik_element in root.findall("ucenik"):
            ime = ucenik_element.find("ime").text
            prezime = ucenik_element.find("prezime").text
            razred = ucenik_element.find("razred").text
            ucenik = Ucenik(ime, prezime, razred)
            Ucenik.ucenici.append(ucenik)
        self.osvjezi_listu_ucenika()   
    ...
    def __init__(self, root):
        ...
        self.gumbi_frame.grid(row=2, column=0, sticky="ew")
        self.gumbi_frame.columnconfigure((0, 1), weight=1)
        self.spremi_csv_button = tk.Button(self.gumbi_frame, text="Spremi u CSV", command=self.spremi_u_csv)
        self.spremi_csv_button.grid(row=0, column=0, padx=5, sticky="ew")
        self.spremi_xml_button = tk.Button(self.gumbi_frame, text="Spremi u XML", command=self.spremi_u_xml)
        self.spremi_xml_button.grid(row=0, column=1, padx=5, sticky="ew") 
        self.ucitaj_csv_button = tk.Button(self.gumbi_frame, text="Učitaj iz CSV-a", command=self.ucitaj_iz_csv)
        self.ucitaj_csv_button.grid(row=1, column=0, padx=5, pady=5, sticky="ew")
        self.ucitaj_xml_button = tk.Button(self.gumbi_frame, text="Učitaj iz XML-a", command=self.ucitaj_iz_xml)
        self.ucitaj_xml_button.grid(row=1, column=1, padx=5, pady=5, sticky="ew")
