...
class RezervacijaApp:
    ...
    def __init__(self, root):
        ...
        tk.Button(
            ...
            command=self.obrisi_rezervaciju,
            bg="#BB6065",
            fg="white",
            font=("Arial", 12, "bold")
        ).pack(pady=5)
    ...
    def show_stolovi(self):
        ...
        try:
            ...
            messagebox.showerror("Greška", "Slika 'raspored.png' nije pronađena.")
            win.destroy()
    ...
    def spremi_rezervaciju(self):
        ...
        messagebox.showinfo("Uspjeh", "Rezervacija je spremljena.")
        self.win_rez.destroy()


    def obrisi_rezervaciju(self):
        selekcija = self.lista.curselection()

        if not selekcija:
            messagebox.showwarning("Upozorenje", "Odaberite rezervaciju za brisanje!")
            return

        if not messagebox.askyesno("Potvrda", "Jeste li sigurni da želite obrisati rezervaciju?"):
            return

        index = selekcija[0]
        self.rezervacije.pop(index)

        self.spremi_xml()
        self.osvjezi_listu()


    def inicijaliziraj_xml(self):
